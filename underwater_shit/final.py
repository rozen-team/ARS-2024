...
def keep_yaw(set_yaw, speed=0):
    try:
        ...
        auv.set_motor_power(2, -output  + speed)
        auv.set_motor_power(1, output  + speed)
        return error
...
def keep_depth(depth_to_set, speed_to_depth=1.0):
    try:
        try:
            ...
            output = clamp(output * speed_to_depth, 25, -25)
            output += 10
...
while True:
    ...
    while True:
        ...
        # coord = coord.sort()
        if action_flag is not None and time.time() - action_flag > 1:
            print("LEFT")
            rotate(initial_yaw+90, d, 20)
