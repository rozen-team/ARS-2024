...
Kp_depth = 100  # кофецент пропорционального регулятора на глубину
Kd_depth = 20  # кофецент дифференциального регулятора на глубину
Kp_yaw = 0.5  # кофецент пропорционального регулятора на курс
Kd_yaw = 0  # кофецент дифференциального регулятора на курс

Low_hsw_orange = (160, 20, 50)  # рамки бинаризации для желтый
Max_hsv_orange = (180, 255, 255)
...
def motors_up():
    #    motor_control_regulator_s(5, Yaw_const, 0.3, 0)   
    motor_control_regulator_s(5, auv.get_yaw(), 0.1, 5)
    motor_control_regulator_s(5, auv.get_yaw(), -0.3, 0)
...
def keep_yaw(yaw_to_set, speed_to_yaw=0):  # ПД регулятор по курсу, !!! без ожидания будет выдавать ошибку !!!
    try:
        ...
        output = keep_yaw.regulator.process(error)  # забиваем ошибку и получаем выходное значение на моторы
        output = clamp(output, 100, -100)  # проверяем выходное значение на ограничение
        auv.set_motor_power(1, clamp((speed_to_yaw - output), 100, -100))  # передаём выходное значение на мотор 0
        auv.set_motor_power(2, clamp((speed_to_yaw + output), 100, -100))  # передаём выходное значение на мотор 1
...
def keep_depth(depth_to_set, depth_SECRET=False):  # ПД регулятор по глубине, !!! без ожидания будет выдавать ошибку !!!
    speed_to_depth = 1
    try:
        time.sleep(0.001)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if data:
            motor_control_regulator(0, data[2], 1.5, 0)
